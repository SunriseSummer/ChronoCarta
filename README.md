...
前端基于 **React 19 + Vite 8 + Tailwind CSS 4** 构建；后端使用 **Express 5 + better-sqlite3** 提供 REST API。
...
- **AI 文案生成** — BYOK 模式调用 OpenAI 兼容接口，基于地点信息 + 图片流式生成文案
- **风格标签** — 内置 13 种文案风格标签（人文地理、实用攻略、吐槽日记、赞美河山、历史沧桑、文化民俗、美食诱惑、自由旷野、摄影视角、小表情、古诗引用、小红书风等）可组合，支持自定义提示词
- **草稿模式** — 新建地点可先保存为草稿（仅创建者可见），确认后再正式发布
- **EXIF 拍摄时间** — 上传照片时自动从 EXIF 提取拍摄时间（`taken_at`），支持前端展示
